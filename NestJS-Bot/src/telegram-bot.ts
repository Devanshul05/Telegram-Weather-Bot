...
export class TelegramBotService {
	...
	constructor(private readonly adminService: AdminService, private readonly userService: UserService,) {
		...
		// Schedule the sendWeatherUpdates function to run every day at 7:00AM
		cron.schedule('0 7 * * *', () => {
	...
	private registerCommands() {
		...
		this.bot.onText(/\/start/, async (msg) => {
			...
			if (existingUser && existingUser.isBlocked) this.bot.sendMessage(chatId, 'You are blocked! Please contact owner for further details.');
			else this.bot.sendMessage(chatId, `Hi ${first_name}, welcome to the Daily Weather Bot, you can subscribe by using the /subscribe command, and unsubscribe using /unsubscribe command.`)
		})
		this.bot.onText(/\/subscribe/, async (msg) => {
			...
			if (existingUser) {
				if (existingUser.isBlocked) this.bot.sendMessage(chatId, 'You are blocked! Please contact owner for further details.');
				else this.bot.sendMessage(chatId, 'You are already registered.');
			} else {
	...
	private async sendWeatherUpdate(chatId: number) {
		...
		const existingUser = await this.userService.getUserByChatId(chatId);
		if (existingUser.isBlocked) return;
